...
const Dashboard = () => {
    ...
    // ==========================================
    // SHARED STATE (For Detail/List Views)
    // ==========================================
    ...
    // ==========================================
    useEffect(() => {
        // Only load infrastructure data if we are NOT in Admin/AI mode
    ...
    // ==========================================
    // DATA FETCHERS (Infrastructure Only)
    // ==========================================
    const loadInfrastructureList = async () => {
        ...
        try {
            ...
            // Frontend filter to ensure we only show the correct category
    ...
    const fetchMetrics = async (id) => {
        try { const res = await api.get(`/core/devices/${id}/metrics/`); setMetrics(res.data || []); } catch (e) {}
    };
        ...
        try {
            ...
            const cleanData = records.map(r => {
                ...
                Object.keys(newR).forEach(k => {
                   if (typeof newR[k] === 'string' && newR[k].includes('%')) newR[k] = parseFloat(newR[k].replace('%',''));
                });
                return newR;
            });

            if (cleanData.length > 0 && currentMetrics.length === 0) {
                const auto = Object.keys(cleanData[0])
                    .filter(k => !['id','timestamp','hostname'].includes(k) && typeof cleanData[0][k] === 'number')
                    .map((k, i) => ({ id: i, json_path: k, label: k.toUpperCase(), color: COLORS[i % COLORS.length] }));
                setMetrics(auto);
            }
            setTelemetry(cleanData.reverse());
    ...
    const renderSidebar = () => (
        <div style={styles.sidebar}>
            ...
            <div style={styles.logout} onClick={() => {localStorage.clear(); window.location.href='/login'}}>
                    ...
                    {devices.map(d => (
                        <div key={d.id} style={styles.card} onClick={() => navigate(`/infrastructure/${category}/${d.id}`)}>
                            <div style={{display:'flex', justifyContent:'space-between', marginBottom:'10px'}}>
                                ...
                                <span style={styles.ipBadge}>{d.ip_address}</span>
                            </div>
                            <div style={{display:'flex', justifyContent:'space-between', fontSize:'0.85rem', color:'#6b7280'}}>
                                <span>CPU: {d.latest_snapshot?.cpu_1min || 'N/A'}%</span>
                                <span style={{color:'#10b981', display:'flex', alignItems:'center', gap:'4px'}}><Activity size={12}/> Healthy</span>
                            </div>
                        </div>
                    ))}
                </div>
        ...
        <>
            ...
            <div style={styles.topBar}>
                ...
                <div style={styles.badges}>
                    <span style={styles.ipBadge}>{selectedDevice?.ip_address}</span>
                ...
                <div style={{width:'100%', height:300}}>
                    <ResponsiveContainer>
                        <LineChart data={telemetry}>
                            ...
                            <Legend/>
                            {(metrics || []).map((m, i) => (
                                <Line key={m.id || i} type="monotone" dataKey={m.json_path} name={m.label} stroke={m.color || COLORS[i%COLORS.length]} dot={false} strokeWidth={2}/>
                            ))}
                        </LineChart>
                    </ResponsiveContainer>
                </div>
