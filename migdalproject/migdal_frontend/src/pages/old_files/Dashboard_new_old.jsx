...
const Dashboard = () => {
        ...
        try {
            ...
            const flatData = rawRecords.map(r => {
                ...
                for (let key in r.payload) {
                    ...
                    // NEW: Backwards compatibility to clean up old string metrics
    ...
    const calculateHealth = (device) => {
        ...
        // We look for common keys used in servers/hypervisors
    ...
    const handleLogout = () => {
        // 1. Clear all local storage (Tokens, User Info)
        localStorage.clear();
        
        // 2. Redirect to Login using React Router (No page reload)
        navigate('/login');
    };
                ...
                <div style={{width:'100%', height:300}}>
                    <ResponsiveContainer>
                        <LineChart data={telemetry} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                            ...
                            <XAxis 
                                // 1. Double check: Is your key 'timestamp' or 'report_timestamp'?
                                // Based on our ingest logic, it should be 'timestamp'
                                dataKey="timestamp" 
                                // tickFormatter={(timeStr) => {
                                //     if (!timeStr) return "";
                                //     const date = new Date(timeStr);
                                //     // 2. Safety check: If the date is invalid, return the raw string
                                //     return isNaN(date.getTime()) 
                                //         ? timeStr.substring(11, 16) // Fallback: try to just grab HH:mm from string
                            ...
                            {Array.isArray(metrics) && metrics.map((m, i) => {
                                // Remove '$.' to match the flattened keys in our database
